...
def solve():
    def dijkstra(source):
        ...
        while q:
            ...
            for direction_v, v in enumerate([(i - 1, j), (i + 1, j), (i, j - 1), (i, j + 1)]):
                ...
                # # cannot turn earlier than 4 cells in one direction
                if direction_v != direction_u and nd_u < 4 and nd_u != -1:
                    continue
    ...
    for (u, direction, nd), cost in dist.items():
        if u[0] == len(m) - 1 and u[1] == len(m[0]) - 1 and nd >= 4:
            ans = min(ans, cost)
    print(ans)

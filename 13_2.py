def is_horizontally_symmetric(pattern, center):
    ...
    return sum(pattern[center + i][j] == pattern[center - 1 - i][j] 
               for j in range(len(pattern[0]))
               for i in range(size)) == len(pattern[0]) * size - 1

def is_vertically_symmetric(pattern, center):
    size = min(center, len(pattern[0]) - center)
    return sum(pattern[i][center - 1 - j] == pattern[i][center + j]
               for i in range(len(pattern))
               for j in range(size)) == len(pattern) * size - 1
